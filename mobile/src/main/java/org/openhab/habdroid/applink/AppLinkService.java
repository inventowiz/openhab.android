...
public class AppLinkService extends Service implements IProxyListenerALM {
	public String TAG = "Applink Service";
	...
	*/
	...
	public void startProxy() {
		if (proxy == null) {
			try {
				Log.v(TAG, "Attempting to create proxy service.");
				proxy = new SyncProxyALM(this, "Hello AppLink", false, "438316430"); //438316430
			} catch (SyncException e) {
				e.printStackTrace();
				// error creating proxy, returned proxy = null
				Log.e(TAG, "Failed to create proxy");
				if (proxy == null) {
					stopSelf();
				}
			}
		...
		switch(notification.getHmiLevel()) {
			case HMI_FULL:
				...
				if (notification.getFirstRun()) {
					...
					// send display layout
					try {
						proxy.setdisplaylayout("TILES_ONLY",autoIncCorrId++);
					} catch (SyncException e) {
						DebugTool.logError("Failed to set display layout", e);
					}
					// send addcommands
					try {
						proxy.addCommand(1,"TestItem",autoIncCorrId++);
					} catch (SyncException e) {
						e.printStackTrace();
					}
	...
	public void onOnLockScreenNotification(OnLockScreenStatus notification) {
		...
		Log.i(TAG,"onLockScreenNotification " + displayLockScreen.name());
